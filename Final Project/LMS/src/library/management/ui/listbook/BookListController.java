...
public class BookListController implements Initializable {
    ...
    private void handleBookDeleteOption(ActionEvent event) {
        ...
        if (selectedForDeletion == null) {
            AlertScene.showErrorMessage("No book selected", "Please select a book for deletion.");
            return;
        }
        if (DataAccessHelper.getInstance().isBookAlreadyIssued(selectedForDeletion)) {
            AlertScene.showErrorMessage("Cant be deleted", "This book is already issued and cant be deleted.");
            return;
        }
        ...
        if (answer.get() == ButtonType.OK) {
            Boolean result = DataAccessHelper.getInstance().deleteBook(selectedForDeletion);
            if (result) {
                AlertScene.showSimpleAlert("Book deleted", selectedForDeletion.getTitle() + " was deleted successfully.");
                list.remove(selectedForDeletion);
            } else {
                AlertScene.showSimpleAlert("Failed", selectedForDeletion.getTitle() + " could not be deleted");
            }
        } else {
    ...
    private void handleBookEditOption(ActionEvent event) {
        ...
        if (selectedForEdit == null) {
            AlertScene.showErrorMessage("No book selected", "Please select a book for edit.");
            return;
        }
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/library/management/ui/addbook/add_book.fxml"));
    ...
    public static class Book {
        ...
        private final boolean  availabilty;
